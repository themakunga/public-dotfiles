...
M.plugin = function()
  ...
  if not ok then
    vim.notify('[CHECK REQUIRE FAILED] conform ' .. debug.getinfo(2).source, vim.log.levels.WARN)
    return
  end
  ...
  -- Create user command to match the `cmd` lazy-loading behavior of the original setup
