...
local function on_attach(bufnr)
 ...
 local function opts(desc) 
  ...
  KM.map({mode = "n", motion = 'e', cmd = nil , opts = { buffer = bufnr }})
  ...
  KM.bulk_map({
    ...
    {mode = "n", motion = 'p', cmd = api.node.open.preview, opts = opts("Preview")},
    ...
    {mode = "n", motion = 'O', cmd = function() 
      local node = api.tree.get_node_number_cursor()
      if node then
        vim.ui.open(node.absolute_path)
      end
    end, opts = opts("Open in File Explorer")},
